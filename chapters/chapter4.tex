...
Flan exercises the full feature set of the library: type checking, diagnostics, hover messages, semantic highlighting, go-to-definition, and rename; it is the primary integration example.
LambdaPi serves as a minimal counterpart: it provides only \texttt{TokenizableSig} and \texttt{TokenizablePat} instances for syntax highlighting, with all other classes left at their no-op defaults, showing how little is needed to reach a first running server.
